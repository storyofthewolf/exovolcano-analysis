...
- **Claude CAN reach Discover when the ControlMaster is live.** Claude Code runs as the local user with the real `$HOME`, so it inherits `~/.ssh/config` and the control sockets in `~/.ssh/sockets/`. With a master open, `ssh -o BatchMode=yes discover.nccs.nasa.gov '<cmd>'` works from Claude's Bash tool with no TTY and no 2FA prompt — the master already carries the authentication.
  - Check liveness without opening a connection: `ssh -O check discover.nccs.nasa.gov` → `Master running (pid=…)`. If it reports no master, the user must re-establish it (`ssh -fN etwolf@discover.nccs.nasa.gov`); Claude cannot, since that needs interactive 2FA.
  - Always pass `-o BatchMode=yes` so a dead master fails immediately instead of hanging on a prompt that can never be answered.
  - **Remote commands run under tcsh** — the same hazard as `fetch_exovolc.py` (see above). `ssh host "… 2>/dev/null"` does not suppress stderr; wrap it: `ssh host "bash -c '…'"`.
  - **Read-only vs. write.** Inspection (`squeue`, `tail` a log, `ls`, `du`) is fine unprompted when the user asks about job state. Anything that submits, writes, or deletes — `sbatch`, `scancel`, `rm`, writing under `/gpfsm/dnb07/projects/p54/` — consumes a shared allocation and a shared filesystem: confirm with the user first, every time, even when access obviously works.
  - Simulations themselves are still run by the user, not Claude. This entry is about inspecting and managing results, not about launching CESM.
